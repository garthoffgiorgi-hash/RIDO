...
| `src/components/ui/` | Brand primitives: `Button`, `Card`, `Input`, `Badge`, `Avatar`, `Sheet`, `Fare` (+ `formatCents` — the one place cents become a string), `FareChip`, `SegmentedControl` |
| `src/components/domain/` | RIDO-specific: `MarketingNav`, `MarketingFooter`, `Wordmark`, `RiderTopBar`, `RideCard`, `RideMap`, `PlaceSearch`, `CardForm`, `TierProgress` |
...
| `src/lib/fares/` | Reads the active `fare_rate_cards` row and calls `quoteFare()` — the DB half of ADR-0009, same pattern `src/lib/drivers/server.ts` uses for its own table. `getPaymentPolicy()` reads the same row's three payment columns (hold buffer, cancellation fee and grace) for ADR-0017/ADR-0018 |
