...
**RLS:** a rider reads their own rides, a driver reads rides where they're the driver. Still no `INSERT`/`UPDATE` grant for `authenticated` at all, deliberately — the rider booking flow that now exists writes through the service role instead (`../decisions/0012-rider-books-server-owns-the-write.md`), so there was never a real transition to write a policy for. Same posture will decide the driver-accept write once it exists.
