...
Plus **`accepting_rides`** (bool, default `true`, ADR-0019) — the driver's own online/offline state. The UI calls it Online/Offline; it gates *accepting only*, so an offline driver still sees the whole open board and still finishes a ride they already hold. Plus **`market`** (`text not null default 'san-diego'`) — same shape as `fare_rate_cards.market`, no `CHECK` enumerating known markets (a new one is a row, never a migration); written by whatever creates the row, same as every other vetting fact here.
...
The first pass-through now exists, and the other two deliberately do not. ADR-0024 added
`fare_rate_cards.access_for_all_fee_cents` (the amount, seeded per market) and
`rides.access_for_all_fee_cents` (what a given ride was quoted), so `rider_total_cents` finally
diverges from `fare_cents` and `rides_rider_total_covers_pass_throughs` enforces that the total
covers it. That check is `>=` rather than `=` on purpose: the second pass-through must not require
dropping and recreating it.

What is still missing is the **general** per-line-item breakdown. `FareQuote` carries
`FareLineItem`s and exactly one of them is persisted, into a column named for it. A second
pass-through scoped by something other than `market` — an airport geofence — is what would buy a
child table; a column on a market's card cannot express one. Recorded as the revisit trigger in
ADR-0024 rather than built ahead of need.

The **PUCTRA user fee needs no schema at all** and that is the decision, not an omission: it is
0.1% of aggregate quarterly revenue with a minimum floor, so it is a report over completed rides
rather than a per-ride amount. Accruing it per ride would produce a figure that is wrong in the
direction of looking precise.
