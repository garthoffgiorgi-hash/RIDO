...
`id` · `rider_id` → `auth.users` directly (no `riders` table exists) · `driver_id` → drivers · `status` (`requested`|`accepted`|`in_progress`|`completed`|`canceled`) · `fare_cents` · **`commission_rate_bps`** (snapshot, basis points) · **`commission_cents`** (snapshot) · **`driver_payout_cents`** (snapshot) · `pickup_lat/lng` · `dropoff_lat/lng` · `requested_at` · `accepted_at` · `completed_at` · `created_at`. Two `CHECK` constraints enforce the commission columns exist iff `status='completed'`, and `commission_cents + driver_payout_cents = fare_cents` exactly. A trigger blocks any rewrite of the three commission columns once set.
