...
card move on their own, without a reload. Not built: online/offline toggle, MTD tier-progress
visualization, driver decline, dispatch or proximity matching, and realtime on the driver's *open
pool* — that one is a whole-table subscription with its own authorization question, deferred in
ADR-0020 rather than skipped.
...
even call `listOpenRequests` while a driver holds a live ride. The current-ride card **is** realtime;
the open pool is not, so a driver still sees a request disappear (someone else took it) only on
reload or their next accept attempt. ADR-0020 says why the two are split.
