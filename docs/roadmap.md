...
| Supabase | **Project live, schema applied locally** (ADR-0022's three tables, `driver_availability_log`, and `rides`/`drivers`' new `market` column are proved against a rebuilt Postgres; live-project push is the user's next step, same as every migration before it). Thirty-one migrations, still **fourteen tables** — `market` is a column, not a new one, `text not null default 'san-diego'` on both `rides` and `drivers`, matching `fare_rate_cards.market`'s shape (no `CHECK` enumerating markets; a new one is a row, not a migration). Twenty-one functions, RLS on every table — including the schema's first cross-party policies, `exists`-based per ADR-0013's nullable-column lesson — twenty-four pgTAP files (215 assertions) plus four standalone concurrency proofs, all green, each new assertion confirmed to fail when its property is inverted. `commission_tiers` and `fare_rate_cards` seeded. **`database.types.ts` predates `market`**, same gap `rider_profiles`/`driver_public_profiles`/`ride_ratings` had before their own regeneration: `requestRide()`'s insert payload carries a narrow `& { market: string }` intersection rather than the blanket `UntypedTables`/`as` cast the other three tables still use in `apps/web/src/lib/riders/server.ts` and `apps/web/src/lib/rides/server.ts` — all four gaps close together at the next `npm run types:generate`. |
