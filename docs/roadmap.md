...
| Supabase | **Project live, schema applied locally** (ADR-0022's three tables and `driver_availability_log` are proved against a rebuilt Postgres; live-project push is the user's next step, same as every migration before it). Thirty migrations — the twenty-six through `alter publication supabase_realtime add table rides`, plus `rider_profiles`, `driver_public_profiles` (+ its `SECURITY DEFINER` sync trigger off `drivers`, required since `accepting_rides` already carries a live `authenticated` grant), `ride_ratings` (+ its completion-and-pairing guard, + its aggregate-bump trigger), and **`driver_availability_log`** — the history `accepting_rides` never had, trigger-written and `SECURITY DEFINER` for the same reason, read back through `driver_online_seconds()` (ADR-0022 tier 2). **Fourteen tables**, twenty-one functions, RLS on every table — including the schema's first cross-party policies, `exists`-based per ADR-0013's nullable-column lesson — twenty-three pgTAP files (207 assertions) plus four standalone concurrency proofs, all green, each new assertion confirmed to fail when its property is inverted. `commission_tiers` and `fare_rate_cards` seeded. **`database.types.ts` covers all fourteen tables**; what remains is the narrow `UntypedTables` bridge in `apps/web/src/lib/riders/server.ts` and `apps/web/src/lib/rides/server.ts`, now retirable. |
