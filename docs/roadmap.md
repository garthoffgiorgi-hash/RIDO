...
| Supabase | **Project live, schema applied locally** (ADR-0022's three tables, `driver_availability_log`, and `rides`/`drivers`' `market` column and ADR-0024's `access_for_all_fee_cents` are proved against a rebuilt Postgres; the live-project push of those last two is the user's next step, same as every migration before it). Thirty-two migrations, still **fourteen tables** — `market` is a column, not a new one, `text not null default 'san-diego'` on both `rides` and `drivers`, matching `fare_rate_cards.market`'s shape (no `CHECK` enumerating markets; a new one is a row, not a migration). Twenty-one functions, RLS on every table — including the schema's first cross-party policies, `exists`-based per ADR-0013's nullable-column lesson — twenty-five pgTAP files (225 assertions) plus four standalone concurrency proofs, all green, each new assertion confirmed to fail when its property is inverted. `commission_tiers` and `fare_rate_cards` seeded. **`database.types.ts` predates `market` and `access_for_all_fee_cents`**, same gap `rider_profiles`/`driver_public_profiles`/`ride_ratings` had before their own regeneration: `requestRide()`'s insert payload carries narrow `& { market: string }` and `& { access_for_all_fee_cents: number }` intersections rather than the blanket `UntypedTables`/`as` cast the other three tables still use in `apps/web/src/lib/riders/server.ts` and `apps/web/src/lib/rides/server.ts` — all four gaps close together at the next `npm run types:generate`. |
...
volume, `pickup_geog` is null on every row regardless, ADR-0011) · instant removal of a ride another
