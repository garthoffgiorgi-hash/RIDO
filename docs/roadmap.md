...
Still missing: dispatch/proximity, realtime (every state change appears on reload),
...
volume, `pickup_geog` is null on every row regardless, ADR-0011) · realtime (a
rider or driver still only learns of a state change on reload) · driver app ·
MTD tier-progress visualization · a driver-side notification when a rider cancels · un-declining a
