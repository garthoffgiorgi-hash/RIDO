...
**Last verified: 2026-09-10** (`main`, after #54/#55/#56 merged) — every count in this file
...
Still missing: dispatch/proximity and cash rides — the latter deliberately deferred, because cash
inverts the flow into a driver-owes-RIDO debt that needs its own design. Realtime is now on all
three surfaces (ADR-0020, ADR-0021), with one honest limit: the open-pool board hears arrivals but
**cannot** hear removals, since a taken ride leaves the other drivers' RLS visibility at that
instant.
...
| `packages/pricing` | **Implemented and tested.** Fare quoting (`quoteFare`) and the Prop 22 floor alongside the commission math. Bracketed commission, tier validation, flat-fee resolution, the authorization hold (`holdAmountCents`), and `tierPositionFor()` for the driver's tier-progress card. 111 tests passing identically under **both** Node and Deno. Exact integer arithmetic throughout — no floating-point value anywhere in the path. Reproduces all three figures the docs published by hand ($200.12 at $1,001; $488/$3,112/13.56% at $3,600). |
| Brand | `design-system.md`, `brand-guide.md`, two Design export bundles with handoff notes |
| Supabase | **Project live, schema pushed live.** Thirty-two migrations, **fourteen tables** — `market` is a column, not a new one, `text not null default 'san-diego'` on both `rides` and `drivers`, matching `fare_rate_cards.market`'s shape (no `CHECK` enumerating markets; a new one is a row, not a migration). Twenty-one functions, RLS on every table — including the schema's first cross-party policies, `exists`-based per ADR-0013's nullable-column lesson — twenty-five pgTAP files (225 assertions) plus four standalone concurrency proofs, all green, each new assertion confirmed to fail when its property is inverted. `commission_tiers` and `fare_rate_cards` seeded, including ADR-0024's `access_for_all_fee_cents`, verified via the live project's own SQL Editor. **`database.types.ts` is current** — regenerated against the live schema, closing all four gaps at once: `market`, `access_for_all_fee_cents`, and the `rider_profiles`/`driver_public_profiles`/`ride_ratings` trio. The narrow `& { market: string }`/`& { access_for_all_fee_cents: number }` intersections in `requestRide()`'s insert payload and the `UntypedTables`/`as` cast in `apps/web/src/lib/riders/server.ts` and `apps/web/src/lib/rides/server.ts` are all retired. |
...
volume, and `pickup_geog` only fills once ADR-0029's flag is on) · instant removal of a ride another
driver took (arrivals are live as of ADR-0021; removals need broadcast-from-trigger, costed there
and deferred) · a driver's live location on the rider's map (no GPS trace exists — ADR-0011) · push
notifications · driver app · un-declining a request, and declines expiring (ADR-0019 makes both
trivially addable and neither is built).

`complete-ride` is **deployed** to the live project (`supabase functions deploy complete-ride
--project-ref <ref> --use-api`) and has been called by the app end to end. **Every migration is now
applied to the live project and `database.types.ts` regenerated against it** — including the
realtime publication, so both realtime surfaces work against the live database with no further
push. Deploying the function originally surfaced a real gap: `functions deploy`
...
functionality of its own (the open-request board). ✅ **Post-login landing is ride-aware**
(ADR-0023) — `/auth/landing` sends a signed-in visitor with a live ride to `/request` or `/drive`
directly, never by identity, and `/account` otherwise, unchanged from before.
...
⬜ A `quote-ride` Edge Function. **No longer blocked on anything** — `measureRoute()` supplies
distance and duration, `apps/web/src/lib/maps/route.ts` is Next-free with `.ts`-extensioned imports
so Deno can import it directly, and `/request` now asks for real quotes on every change. It just
isn't needed: `quoteRideRequest()` does the job inside the app, and a function would only start
earning its keep once a non-web caller wants the same path.
...
and never a ride already accepted (ADR-0019). ✅ **Month-to-date tier progress** — `TierProgress`
shows the graduated bands filling, this month's earnings, and the current and next rate, read
straight from `driver_monthly_stats`. ✅ **Rider identity and ratings (ADR-0022)** close `rate` —
live-ride-only driver/rider cards, two-directional `ride_ratings`. ⬜ Dispatch/proximity remains.
...
needing **no schema** (a report over completed rides); ⬜ the airport fee needs ADR-0029's coordinates flipped on. ✅ **Deployable today, `RIDES_LIVE` unset** — booking/accepting/online refused server-side (ADR-0026, `architecture/deployment.md`); neither blocker below has moved.
