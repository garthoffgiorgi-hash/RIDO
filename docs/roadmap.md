...
Still missing: driver decline, dispatch/proximity, realtime on the driver's *open pool* (both
live-ride surfaces are realtime as of ADR-0020; the board is a whole-table subscription deferred
there on purpose), and cash rides — deliberately deferred, because cash inverts the flow into a
driver-owes-RIDO debt that needs its own design.
...
volume, `pickup_geog` is null on every row regardless, ADR-0011) · driver decline · realtime on the
driver's open pool (the two single-row live-ride surfaces have it; the board's whole-table
subscription is deferred in ADR-0020) · a driver's live location on the rider's map (no GPS trace
exists — ADR-0011) · push notifications · driver app · online/offline toggle · MTD tier-progress
visualization.
