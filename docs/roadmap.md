...
**Last verified: 2026-09-04** (branch `claude/rider-identity`) — every count in this file
...
| Supabase | **Project live, schema applied locally** (ADR-0022's three tables are proved against a rebuilt Postgres; live-project push is the user's next step, same as every migration before it). Twenty-nine migrations — the twenty-six through `alter publication supabase_realtime add table rides`, plus `rider_profiles`, `driver_public_profiles` (+ its `SECURITY DEFINER` sync trigger off `drivers`, required since `accepting_rides` already carries a live `authenticated` grant), and `ride_ratings` (+ its completion-and-pairing guard, + its aggregate-bump trigger). **Thirteen tables**, nineteen functions, RLS on every table — including the schema's first cross-party policies, `exists`-based per ADR-0013's nullable-column lesson — twenty-two pgTAP files (191 assertions) plus four standalone concurrency proofs, all green. `commission_tiers` and `fare_rate_cards` seeded. **`database.types.ts` is one run behind again**: the three new tables bridge through the same narrow `UntypedTables` pattern that once covered `ride_declines`, in `apps/web/src/lib/riders/server.ts` and `apps/web/src/lib/rides/server.ts`. |
